...
function FactoriesOrdersForm(props) {
  ...
  useEffect(() => {
    ...
    setClientId('6255f582ae9de4a4a5931c3a');
  }, []);

  useEffect(async () => {
    if (clientId !== '') {
      let orders = await axios.get(
        `http://${secrets.LOCALHOST_IP}:${secrets.TOMCAT_PORT}/sales-system/sellers?get=true&clientOrdersNoClientId=${localStorage.getItem('userName')}`
      );
      orders = orders.data;
  ...
  const handleSavePendingOrders = async () => {
    ...
    try {
      const updatePendingOrdersRes = await axios.put(
        `http://${secrets.LOCALHOST_IP}:${secrets.TOMCAT_PORT}/sales-system/sellers?updateOrders=true`,
        updatedOrders
      );
  ...
  const handlePayOrder = async (orderIndex) => {
    ...
    const payOrderRes = await axios.put(
      `http://${secrets.LOCALHOST_IP}:${secrets.TOMCAT_PORT}/sales-system/sellers?payOrder=${completedOrders[orderIndex]._id}`,
      {}
    );
...
export default FactoriesOrdersForm;
